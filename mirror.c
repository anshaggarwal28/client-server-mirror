...
void pclientrequest(int clientDescriptor)
{
    ...
    while (1)
    {
        ...
        if (val <= 0)
        {
            printf("\nClient has been killed or error while reading the command from the client");
        ...
        {
            printf("Client has requested to break the connection\n");
...
int main(int argc, char *argv[])
{
    ...
    int programMirrorDescriptor, client, portNumber;
    struct sockaddr_in programMirrorAddress;

    // Creating the socket for the program
    if ((programMirrorDescriptor = socket(AF_INET, SOCK_STREAM, 0)) < 0)
    {
        perror("Unable to create the socket for the program");
        exit(1);
    }

    programMirrorAddress.sin_family = AF_INET;
    programMirrorAddress.sin_addr.s_addr = htonl(INADDR_ANY);
    programMirrorAddress.sin_port = htons(PORT);

    // Binding the socket mirror
    bind(programMirrorDescriptor, (struct sockaddr *)&programMirrorAddress, sizeof(programMirrorAddress));
    // listen
    listen(programMirrorDescriptor, 50);

    printf("\nWaiting for the 1st client to connect with the mirror and execute the commands\n");
    // Infinite loop to wait for the different clients
    for (;;)
    {
        int childStatus;
        socklen_t clientLen = sizeof(clientId); // Variable to get the client details
        client = accept(programMirrorDescriptor, (struct sockaddr *)&clientId, &clientLen);
        int child = fork();
        if (child == 0)
        {
            // Created a child whenever there is client connection
            printf("Connected to the client using mirror %s:%d\n", inet_ntoa(clientId.sin_addr), ntohs(clientId.sin_port));
        ...
        printf("\nWaiting for the next client now to connect with the mirror and execute the commands\n");
    }
    return 0;
}
